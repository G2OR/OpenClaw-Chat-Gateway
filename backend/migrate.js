...
try {
  ...
  if (legacySession || mainAsSessionId) {
    ...
    db.prepare("UPDATE sessions SET id = ?, name = '綜合管家', agentId = 'main' WHERE id = ?").run(newId, oldId);
    ...
    if (mainAgent) {
      db.prepare("UPDATE sessions SET name = '綜合管家' WHERE agentId = 'main'").run();
      console.log("Updated main agent session name to '綜合管家'");
    } else {
        ...
        db.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?)
        `).run(randId, '綜合管家', 'main', 0, now, now);
