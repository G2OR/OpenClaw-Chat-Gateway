...
app.post('/api/config/restart', async (_req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: '執行重啟命令失敗: ' + error.message });
  }
});
...
app.post('/api/models/test', async (req, res) => {
  try {
    ...
    try {
      ...
      if (resp.ok) {
        return res.json({ success: true, message: '模型有效連通' });
      } else {
      ...
      clearTimeout(timeoutId);
      return res.json({ success: false, error: e.message || '網絡連接失敗' });
    }
  } catch (err: any) {
...
app.post('/api/characters', async (req, res) => {
  ...
  // Validate agentId
  if (!char.agentId) {
    return res.status(400).json({ success: false, error: '智能體 ID 不能為空' });
  }
  if (/\s/.test(char.agentId)) {
    return res.status(400).json({ success: false, error: '智能體 ID 不允許包含空格' });
  }
  ...
  if (isDuplicate) {
    return res.status(400).json({ success: false, error: `智能體 ID "${char.agentId}" 已存在，請使用其他 ID` });
  }
