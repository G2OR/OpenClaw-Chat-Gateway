...
export class DB {
  ...
  private init() {
    this.db.exec(`
      ...
      INSERT OR IGNORE INTO characters (id, name, agentId, systemPrompt) 
      VALUES ('char_coder', '代碼專家', 'coder', 'You are an expert software engineer and architect.');

      -- Insert default commands if they don't exist
      INSERT OR IGNORE INTO quick_commands (command, description) VALUES ('/models', '列出模型供應商可進一步變更模型');
      INSERT OR IGNORE INTO quick_commands (command, description) VALUES ('/help', '幫助信息');
      INSERT OR IGNORE INTO quick_commands (command, description) VALUES ('/clear', '清空當前會話');
    `);
