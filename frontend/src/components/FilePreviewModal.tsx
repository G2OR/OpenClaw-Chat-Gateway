      ...
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center bg-white/80 z-10">
          <div className="flex flex-col items-center gap-3 text-gray-500">
            ...
            <span className="text-sm font-medium">正在渲染文檔...</span>
          </div>
        </div>
      )}
...
export default function FilePreviewModal({ url, filename, onClose }: FilePreviewModalProps) {
    ...
    try {
      ...
      if (!headResponse.ok) {
        if (headResponse.status === 404) {
          setPreview({ status: 'error', message: '文件未找到或已被刪除' });
          return;
        }
        ...
        throw new Error(`無法訪問文件 (${headResponse.status})`);
      }
    } catch (err: any) {
      setPreview({ status: 'error', message: err.message || '網絡請求失敗，文件不可訪問' });
      return;
    }
  ...
  async function loadDocxFallback() {
    try {
      ...
      if (!response.ok) throw new Error(response.status === 404 ? '文件未找到或已被刪除' : `加載失敗 (${response.status})`);
      const arrayBuffer = await response.arrayBuffer();
      const result = await mammoth.convertToHtml({ arrayBuffer });
      setPreview({ status: 'ready', type: 'html', content: result.value });
    } catch (err: any) {
      setPreview({ status: 'error', message: `無法預覽 Word 文檔: ${err.message}` });
    }
  }
  ...
  async function loadXlsxFallback() {
    try {
      ...
      setPreview({ status: 'error', message: `無法預覽表格文件: ${err.message}` });
    }
  }
  ...
  async function loadText(type: 'text' | 'code') {
    try {
      ...
      setPreview({ status: 'error', message: `無法預覽文本文件: ${err.message}` });
    }
  }
      ...
      >
        {preview.status === 'loading' && (
          <div className="flex flex-col items-center gap-4 text-gray-500">
            ...
            <p className="text-sm font-medium">正在加載預覽...</p>
          </div>
        )}
